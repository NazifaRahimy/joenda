...
const Post = ({addToFavorites}) => {
  ...
  const maxTranslate = Math.max(
    0,
    (postProduct.length - visibleBoxes) * boxWidth
  );
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/ads/${id}`)
      ...
      .then((data) => {
        ...
        if (data.category) {
          fetch(
            `https://joyenda-server.onrender.com/ads?category=${data.category}`
          )
            .then((res) => res.json())
            .then((items) => {
              console.log("محصولات مرتبط:", items.ads);
              if (Array.isArray(items.ads)) {
                const filtered = items.ads.filter(
                  (item) =>
                    item.id !== data.id && item.category === data.category
                );
                setRelated(filtered);
              } else {
                console.warn("❌ ads آرایه نیست:", items);
              }
            });
        }
      })
  ...
  return (
    <div className="w-full relative pt-10">
      <button
        onClick={() => navigate(-1)}
        className="absolute top-0 right-5 px-4 text-sm font-medium py-1 rounded-lg dark:hover:bg-cyan-900 dark:hover:text-white m-10 flex justify-center items-center gap-1 hover:text-cyan-700 hover:bg-cyan-100 transition-colors duration-200"
      >
      ...
      <div className="w-auto mx-10 h-auto flex flex-col md:flex-row mt-20 gap-8">
        <div className="w-full md:w-[65%] h-auto">
          ...
          <p className="text-lg  items-center mt-4 flex gap-1">
            <CiCalendar /> <span> {product && timeAgo(product.createdAt)}</span>{" "}
            <span className="text-sm mr-3"> {t("post.in")}</span>{" "}
            {product.region} <span> {product.district}</span> {product.province}{" "}
          </p>
          <p className={`mt-5 text-xl `}>
            {" "}
            Product State: <span className="text-[16px]">like new</span>
          </p>
        ...
        <div className="w-full mt-6 md:mt-0 md:w-[35%] h-auto">
          <div className="flex justify-center gap-3">
            ...
            <button
              ...
              className="w-[50%] px-3 h-9 bg-cyan-600 text-white flex items-center justify-center rounded-md gap-2.5 text-sm font-medium transition-colors duration-200 hover:bg-cyan-500 border-none outline-none"
            >
              <IoHeartOutline size={20} />{" "}
              <span>{t("post.add_to_favorite")}</span>
            </button>
          </div>
          {isLoginOpen && (
            <LoginModal
          ...
          <div className="view flex justify-between mt-5">
            ...
            <span className="inline-flex items-center gap-2.5">
              {product.views || 0} <IoEyeOutline />
            </span>
          </div>
          <div className="flex justify-between mt-5">
            <p>{t("post.posted_by")} :</p>
            <span className="inline-flex items-center gap-2.5 transition-colors duration-200 hover:text-cyan-600">
              {product.seller} <IoLinkOutline />
            </span>
          ...
          <div className="flex justify-between mt-5">
            ...
            <span className="inline-flex items-center gap-2.5 text-gray-300">
              <MdOutlineExpandLess size={22} />
            </span>
          </div>
          <div className="flex gap-2 mt-4">
            <div className="bg-[#282828] hover:bg-[#393939] rounded-md px-3 h-9 text-white inline-flex items-center">
              {t(`create.${product.category}`)}
            </div>
            <div className="bg-[#282828] hover:bg-[#393939] text-white rounded-md px-3 h-9 inline-flex items-center">
              Food items
            </div>
          </div>
          <hr className="my-7" />
          ...
          <div ref={RefDropDown}>
            <button
              ...
              className="inline-flex items-center outline-none gap-2.5 border border-cyan-200 dark:border-cyan-300 dark:hover:bg-cyan-800 rounded-full px-4 shadow py-1.5 hover:text-cyan-700 hover:bg-cyan-100 transition-colors duration-200 dark:text-white"
            >
              <IoShareSocialOutline />
              {t("post.share")}
            </button>
            {open && (
              <div className="w-[300px] border border-cyan-200 rounded-md mt-1 h-[80px] flex justify-center items-center gap-2 dark:border-cyan-300">
                <a
                  ...
                  className="p-3 hover:text-cyan-700 hover:bg-cyan-100 cursor-pointer rounded-md"
                >
                  <IoLogoInstagram />
                </a>
                <a
            ...
            <div
              ...
              style={{
                transform: `translateX(${
                  (translateX - dragOffset) * directionMultiplier
                }px)`,
              }}
            ...
            >
              {related.map((product, index) => (
                <div
                  ...
                  className="shrink-0 w-full md:w-1/2 lg:w-1/4 px-2"
                >
                  <div className="bg-white border border-cyan-200 dark:border-300 shadow-md rounded-md dark:bg-[#282828] h-[360px]">
                    ...
                    <div className="info flex-1 p-4">
                      ...
                      <p className="text-xs mt-1">
                        ...
                        <span className="px-1">{t("post.in")}</span>
                        {product.district}
                        {product.region},{product.province}
                      </p>
                      <p className="text-xs mt-2">{product.location}</p>
                      <p className="text-xs mt-2">Product State: New</p>
                    </div>
                    <div className="contact flex justify-between px-4 mb-2">
                      ...
                      <div className="hidden md:flex items-center gap-2">
                        <button className="text-sm font-medium hover:bg-cyan-100 transition-colors outline-none rounded-full duration-200 hover:text-cyan-700 h-9 dark:hover:bg-cyan-900 dark:hover:text-white">
                          <FiPhone size={20} />
                        </button>
                        <button className="text-sm font-medium hover:bg-cyan-100 transition-colors outline-none rounded-full duration-200 dark:hover:bg-cyan-900 dark:hover:text-white h-9">
                          <FiHeart size={20} />
                        </button>
                      </div>
                    </div>
